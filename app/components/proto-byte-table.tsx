...
function ProtoByteTableRow({ field }: { field: DecodedField }) {
  ...
  if (field.type === "message" && Array.isArray(field.value)) {
    return (
      <>
        <tr>
          <td className="border-2 border-gray-600 px-4 py-2">{formatByteRange(field.byteRange)}</td>
          <td className="border-2 border-gray-600 px-4 py-2">{field.fieldNumber ?? ""}</td>
          <td className="border-2 border-gray-600 px-4 py-2">{field.type}</td>
          <td className="border-2 border-gray-600 px-4 py-2">
            <span className="font-medium text-gray-300">{field.message || "Message"}</span>
