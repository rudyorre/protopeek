...
describe('ProtoVisualizer', () => {
    ...
    const sampleData = JSON.stringify({ type: 'message', message: 'Person', value: sampleDataArray });
    ...
    test('renders top-level fields', () => {
        ...
        expect(screen.getAllByText('1:', { exact: false }).length).toBeGreaterThanOrEqual(1);
        expect(screen.getByText('John Doe', { exact: false })).toBeInTheDocument();
        expect(screen.getAllByText('2:', { exact: false }).length).toBeGreaterThanOrEqual(1);
        // Use getAllByText for '123' since it appears in both id and '123 Main St'
        expect(screen.getAllByText('123', { exact: false }).length).toBeGreaterThanOrEqual(1);
        expect(screen.getAllByText('3:', { exact: false }).length).toBeGreaterThanOrEqual(1);
    ...
    test('renders nested message fields', () => {
        ...
        expect(screen.getAllByText('4:', { exact: false }).length).toBeGreaterThanOrEqual(1);
        // The nested fields (street/city) are fieldNumbers 1 and 2
        expect(screen.getAllByText('1:', { exact: false }).length).toBeGreaterThanOrEqual(2);
        expect(screen.getByText('123 Main St', { exact: false })).toBeInTheDocument();
        expect(screen.getAllByText('2:', { exact: false }).length).toBeGreaterThanOrEqual(2);
